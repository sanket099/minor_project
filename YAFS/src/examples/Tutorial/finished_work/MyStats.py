...
class Stats:
    ...
    def __init__(self, defaultPath="result", topology = None):
    ...
    def compute_times_df(self):
        ...
        # k = self.df["time_out"] - self.df["time_reception"]
        # result = self.get_watt(k, topology, Metrics.WATT_SERVICE)
        # print(" some result" + str(result))

        # nodeInfo = self.topology.get_info

        # node = self.df["time_service"] * nodeInfo[0]["WATT"]
        # self.df["my_power"] = node

    #  self.df["power"]  = result[0]["watt"]

    def times(self, time, topology, value="mean"):
        if "time_total_response" not in self.df.columns:
    ...
    def average_loop_response(self, time_loops):
        """
        There is no check for the existence of the loop: user responsability
        """
        if "time_total_response" not in self.df.columns:
            self.compute_times_df()
    ...
    def get_watt(self, totaltime, topology, by=Metrics.WATT_SERVICE):
        ...
        if by == Metrics.WATT_SERVICE:
            ...
            for node_key in nodeInfo:
                ...
                uptime = end - start
                results[node_key] = {"model": nodeInfo[node_key]["model"],  # "type":nodeInfo[node_key]["type"],
                                     "watt": uptime * nodeInfo[node_key]["WATT"], "uptime": uptime}
    ...
    def get_cost_cloud(self, topology):
        ...
        for id_node in nodes.index:
            if nodeInfo[id_node]["model"] == "cloud":
                results[id_node] = {"model": nodeInfo[id_node]["model"],  # "type": nodeInfo[id_node]["type"],
                                    "watt": nodes.loc[id_node].time_service * nodeInfo[id_node]["WATT"]}
    ...
    def get_latency(self):
        ...
        dic = {"time_latency": self.df["time_latency"],
               "time_wait": self.df["time_wait"],
               "time_service": self.df["time_service"],
               "time_response": self.df["time_response"],
               "time_total_response": self.df["time_total_response"]}
    ...
    def avg_times(self):
        ...
        st = 0
        for i in range(n):
            st += self.df["time_latency"][i]
            print(self.df["time_latency"])
        return st / n

...
def create_application():
    ...
    m_c = Message("M.C", "Sensor", "ServiceA", instructions=20 * 10 ^ 6, bytes=1000)
    m_d = Message("M.D", "ServiceA", "Actuator", instructions=30 * 10 ^ 6, bytes=500, broadcasting=True)

    m_e = Message("M.E", "Sensor", "ServiceA", instructions=20 * 10 ^ 6, bytes=1000)
    m_f = Message("M.F", "ServiceA", "Actuator", instructions=30 * 10 ^ 6, bytes=500, broadcasting=True)
    ...
    a.add_service_module("ServiceA", m_c, m_d, fractional_selectivity, threshold=1.0)
    a.add_service_module("ServiceA", m_e, m_f, fractional_selectivity, threshold=1.0)
...
def create_json_topology():
    ...
    cloud_dev2 = {"id": 3, "model": "cloud","mytag":"cloud", "IPT": 5000 * 10 ^ 6, "RAM": 40000,"COST": 3,"WATT":20.0}

    link1 = {"s": 1, "d": 0, "BW": 1, "PR": 10}
...
def main(simulated_time):
    ...
    pop.set_src_control({"model": "sensor-device", "number": 1, "message": app.get_message("M.C"),
                         "distribution": dDistribution})  # 5.1}})
    pop.set_src_control({"model": "sensor-device", "number": 1, "message": app.get_message("M.E"),
                         "distribution": dDistribution})  # 5.1}})

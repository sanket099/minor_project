...
def main():
    ...
    id_lambdas = list(zip(range(0,len(lambdas_wl)),lambdas_wl))
    ...
    for f in functions:
        ...
        if "Cluster" in f:
            ...
            for idWL in id_lambdas:
                ...
                for dev, value in sorted(centralWL[idx].items(), key=function,reverse=True):
                    # print "%s: %s" % (dev, value)
        ...
        for idx,app in enumerate(apps):
            s.deploy_app(app, placement, pops[idx])
